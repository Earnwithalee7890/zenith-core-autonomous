...
import { fetchPageContent } from "../tools/browser.js";
import { uiLibrary } from "../tools/uiLibrary.js";
  ...
  private getTools(): Record<string, CoreTool> {
    ...
    if (config.tools.codeInterpreterEnabled) {
      tools.code_analysis = tool({
        ...
        execute: async ({ code, language, task }: { code: string, language?: string, task: "explain" | "debug" | "improve" | "review" }) => {
          ...
          return {
            ...
            task,
            note: "Analyze this code with a focus on 'Functionality, Design, and Speed'. If it's a UI task, emphasize modern aesthetics, accessibility, and responsive behavior.",
          };
        },
      });

      tools.fetch_page = tool({
        description: "Fetch the full text content of a webpage from a URL. Use this for deep research when search snippets aren't enough.",
        parameters: z.object({
          url: z.string().describe("The URL of the webpage to fetch"),
        }),
        execute: async ({ url }: { url: string }) => fetchPageContent(url),
      });

      tools.ui_templates = tool({
        description: "Get Zenith's premium UI boilerplates and animation patterns. Use these to deliver a 'WOW' factor that wins hackathons. Supports dashboards, feeds, and glassmorphic designs.",
        parameters: z.object({
          type: z.enum(["tailwind-config", "html-boilerplate", "vite-config", "dashboard", "zenith-animation"]).describe("The type of template or boilerplate to retrieve"),
          title: z.string().optional().describe("Title for the layout (if applicable)"),
        }),
        execute: async ({ type, title }: { type: "tailwind-config" | "html-boilerplate" | "vite-config" | "dashboard" | "zenith-animation", title?: string }) => {
          logger.tool("ui_templates", "start", `Type: ${type}`);
          switch (type) {
            case "tailwind-config": return { content: uiLibrary.getTailwindConfig() };
            case "html-boilerplate": return { content: uiLibrary.getHtmlBoilerplate(title || "Zenith Project") };
            case "vite-config": return { content: uiLibrary.getViteConfig() };
            case "dashboard": return { content: uiLibrary.getDashboardLayout(title || "Zenith Dashboard") };
            case "zenith-animation": return { content: uiLibrary.getZenithAnimation() };
          }
        },
      });
      ...
      tools.create_file = tool({
        description: `Create a file for a high-quality deliverable project (website, app, UI, tool). Use modern tech (e.g., React, TypeScript, Tailwind CSS) unless specified otherwise. Focus on Visual Excellence and Robust Functionality. Call multiple times for multi-file projects, then use finalize_project.`,
        parameters: z.object({
          path: z
            .string()
            .describe(
